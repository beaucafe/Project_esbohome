...
export class DataRepository {
  ...
  async getPoscontrol() {
    // try {
    //   let query = await this.mssql.query(strquery.strPoscontrol())
    //   let data = await this.mssql.dataRecordset(query)
    //   return data
    // } catch (error) {
  ...
  async checkTablePosDaily(config: IConfig, posname: string) {
    try {
      ...
      let query = await pool.query(strquery.dailyTablePos)
  ...
  BeginSetupPosControl = async (
    ...
    try {
      ...
      let query = await POS1.query(strquery.strPoscontrol(option))
      let data = await POS1.dataRecordset(query)

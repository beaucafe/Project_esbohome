...
import { PosnameDto, SETTINGPOSDto, OptionDTO } from "./data.dto"
...
export class DataController {
    ...
    @Post("/MonthByPos")
    getMonthByPos(@Body() option?: OptionDTO) {
        return option === undefined
            ? this.service.monthbypos()
            : this.service.monthbypos(option)
    }
    ...
    updatePosByPos(@Body() posname: SETTINGPOSDto) {
        ...
        try {
            ...
            if (!posname.start && !posname.stop)
                return this.service.dailyTest(name)

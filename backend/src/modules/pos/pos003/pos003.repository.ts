...
export class Pos003Repository {
  ...
  config: setConfig
  constructor(
    @InjectModel('PosData3')
    private posModel: Model<dataSummary>, // private configService: ConfigService,
  ) {
    this.PosName = 'POS3'
    this.config = {
      // user: this.configService.get<string>('posconfig.USERPOS'),
      // password: this.configService.get<string>('posconfig.PASSPOS'),
      // server: this.configService.get<string>('posconfig.SVR_POS3'),
      // database: this.configService.get<string>('posconfig.DBPOS3'),
      user: process.env.USERPOS,
      password: process.env.PASSPOS,
      server: process.env.SVR_POS3,
      database: process.env.DBPOS3,
      pool: {
        max: 10,
        min: 0,
